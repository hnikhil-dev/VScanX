...
def test_prompt_injection_vulnerable(mock_handler):
    ...
    assert any(
        f["severity"] == "CRITICAL" and "Prompt Injection Vulnerability" in f["finding"] for f in result["findings"]
    )
...
def test_code_execution_vulnerable(mock_handler):
    ...
    assert any(
        f["severity"] == "CRITICAL" and "Agent Remote Code Execution" in f["finding"] for f in result["findings"]
    )
...
def test_memory_poisoning_vulnerable(mock_handler):
    ...
    assert any(
        f["severity"] == "HIGH" and "Agent Memory Poisoning Vulnerability" in f["finding"] for f in result["findings"]
    )
...
def test_data_exfiltration_vulnerable(mock_handler):
    ...
    assert any(
        f["severity"] == "HIGH" and "Data Exfiltration Vulnerability" in f["finding"] for f in result["findings"]
    )
